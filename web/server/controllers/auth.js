...
exports.register = (req, res) => {
    ...
    const { username, name, email, birthday, password } = req.body;
    ...
    User.findOne({ email }).exec((err, user) => {
        ...
        const token = jwt.sign({ username, name, email, birthday, password }, process.env.JWT_ACCOUNT_ACTIVATION, {
            expiresIn: '10m'
        });
...
exports.registerActivate = (req, res) => {
    ...
    jwt.verify(token, process.env.JWT_ACCOUNT_ACTIVATION, function(err, decoded) {
        ...
        const { username, name, email, birthday, password } = jwt.decode(token);
        const versID = shortId.generate();
        ...
        User.findOne({ email }).exec((err, user) => {
            ...
            // register new user
            const newUser = new User({ versID, name, username, email, birthday, password });
